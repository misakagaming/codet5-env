DATADIR="multilingual" # Change to your data path.

run() {
  for seed in 1234 2345 3456; do
    #preprocess data
    NAME=$(date +%Y%m%d%H)_MultilingualTrans_many_to_many_seed${seed}_lr${lr}_maxlen1536_warmup200
    OUTPUT_DIR=output/saved_models/MultilingualTrans/many_to_many_${lr}_maxlen1536_seed${seed}/${NAME}
    echo ${NAME}
    mkdir -p ${OUTPUT_DIR}
    ...
    for source_name in "C" "C++" "C#" "Java" "Go" "PHP" "Python" "VB"; do
...
GPUID='0,1,2,3'
lr=3e-5
source_names="C,C++,C#,Java,Go,PHP,Python,VB"

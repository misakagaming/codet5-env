...
def DFG_coolgen(root_node,index_to_code,states):
    ...
    def_statement=[]
    states=states.copy() 
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='note_statement':        
        idx,code=index_to_code[(root_node.start_point,root_node.end_point)]
        if root_node.type==code:
            return [],states
        elif code in states:
        ...
        DFG=[]
        for node in right_nodes:
            temp,states=DFG_coolgen(node,index_to_code,states)
            DFG+=temp
        ...
        for child in root_node.children:
            if 'else' in child.type:
                tag=True
            if child.type not in ['elseif_statement','else_statement']:
                temp,current_states=DFG_coolgen(child,index_to_code,current_states)
                DFG+=temp
            else:
                temp,new_states=DFG_coolgen(child,index_to_code,states)
                DFG+=temp
                others_states.append(new_states)
        others_states.append(current_states)
        if tag is False:
        ...
        for i in range(2):
            ...
            for left_node,right_node in zip(left_nodes,right_nodes):
                ...
                for token1_index in left_tokens_index:
                    ...
                    states[code1]=[idx1]
                DFG+=temp   
            block=root_node.child_by_field_name('block') 
            if  block is not None:
                temp,states=DFG_coolgen(block,index_to_code,states)
                DFG+=temp 
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
        ...
        for i in range(2):
            for child in root_node.children:
                temp,states=DFG_coolgen(child,index_to_code,states)
                DFG+=temp    
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
